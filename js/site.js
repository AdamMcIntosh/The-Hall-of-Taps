    ...
    loadBeers: function (tableEl, paginationEl, pageSize) {
      ...
      function loadAllBeers() {
        ...
        return fetch(DATA + '/beers/all-manifest.json', { cache: 'no-store' })
          .then(function (r) {
            if (!r.ok) return Promise.reject(new Error('No chunked data'));
            return r.json();
          })
          .then(function (manifest) {
            if (!manifest || typeof manifest.chunks !== 'number' || manifest.chunks <= 0) return Promise.reject(new Error('Invalid manifest'));
            var fetches = [];
      ...
      function applyFiltersAndRender() {
        ...
        if (hasActiveFilter()) {
          ...
          loadAllBeers()
            ...
            .catch(function () {
              tableEl.innerHTML = '<tr><td colspan="' + colspan + '" class="error">Could not load beers. Export data from <strong>HallofTaps.Engine</strong> into this site&rsquo;s <code>data</code> folder, then refresh.</td></tr>';
            });
        } else {
